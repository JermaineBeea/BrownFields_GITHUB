...
//
//        default boolean isValidMoveCommand(MoveCommand command, CommandHandler commandHandler) {
//            if (command.arguments.length != 3) {
//                onComplete(new Response("ERROR", "Invalid move format. Use '<direction> <robotName> <steps>'."));
//                return false;
//            }
//
//            String direction = command.arguments[0].toLowerCase();
//            int steps = commandHandler.parseSteps(command.arguments[2]);
//
//            if (!direction.equals("forward") && !direction.equals("back")) {
//                onComplete(new Response("ERROR", "Move direction must be 'forward' or 'back'."));
//                return false;
//            }
//
//            if (steps < 1 || steps > 5) {
//                onComplete(new Response("ERROR", "Steps must be between 1 and 5."));
//                return false;
//            }
//
//            return true;
...
//            case ShutdownCommand shutdownCommand -> handleShutdown(shutdownCommand, clientId, handler);
...
//            completionHandler.onComplete(new Response("ERROR", "Cannot launch more than 2 robots."));
//            return;
//        }
//
...
//
//        if (!handler.isValidMoveCommand(command, this)) return;
//
//        if (!isValidMoveCommand(command, handler)) return;
//
//        String direction = command.arguments[0].toLowerCase();
...
//        if (!visibilityHandler.canRobotMove(robot, handler)) return;
//
...
//
//            if (!status.handleMovementStatus(this, robot, previousPosition, handler)) {
//                return;
//            }
//        }
//
//        Response response = new Response("OK", "Moved " + robot.getName() + " to [" +
//                robot.getPosition().getX() + "," + robot.getPosition().getY() + "]");
//        world.stateForRobot(robot, response);
//        handler.onComplete(response);
//    }
//
//    private boolean handleMovementStatus(Status status, Robot robot, Position previousPosition,
//                                         CompletionHandler handler) {
//        // TODO Auto-generated method stub
//        throw new UnsupportedOperationException("Unimplemented method 'handleMovementStatus'");
//    }
//    private void moveInDirection(Robot robot, String direction) {
//        // TODO Auto-generated method stub
//        throw new UnsupportedOperationException("Unimplemented method 'moveInDirection'");
//    }
//    private boolean canRobotMove(Robot robot, CompletionHandler handler) {
//        // TODO Auto-generated method stub
//        throw new UnsupportedOperationException("Unimplemented method 'canRobotMove'");
//    }
//
//        for (int i = 0; i < steps; i++) {
//        moveInDirection(robot, direction);
//        status = world.isPositionValid(robot.getPosition());
//        if (!handleMovementStatus(status, robot, previousPosition, handler)) {
//            return;
//        }
//    }
//
...
//
//
//private boolean isValidMoveCommand(MoveCommand command, CompletionHandler handler) {
//    if (command.arguments.length < 3) {
//        handler.onComplete(new Response("ERROR", "Invalid move command. Usage: <direction> <name> <steps>"));
//        return false;
//    }
//    String direction = command.arguments[0].toLowerCase();
//    if (!direction.equals("forward") && !direction.equals("back")) {
...
//}
//
//private boolean handleMovementStatus(Status status, Robot robot, Position previousPosition, CompletionHandler handler) {
//    switch (status) {
//        case HitObstaclePIT:
//            robot.status = Robot.RobotStatus.Dead;
//            Response pitResponse = new Response("ERROR", robot.getName() + " fell into a pit and died.");
//            world.stateForRobot(robot, pitResponse);
//            handler.onComplete(pitResponse);
//            return false;
//        case HitObstacle:
//            robot.setPosition(previousPosition.getX(), previousPosition.getY());
//            Response obstacleResponse = new Response("ERROR", robot.getName() + " hit an obstacle.");
//            world.stateForRobot(robot, obstacleResponse);
//            handler.onComplete(obstacleResponse);
//            return false;
//        case OutOfBounds:
//            robot.status = Robot.RobotStatus.Dead;
//            Response outResponse = new Response("ERROR", robot.getName() + " fell off the world.");
//            world.stateForRobot(robot, outResponse);
//            handler.onComplete(outResponse);
//            return false;
//        default:
//            return true;
//    }
...
//            } else if (robot.status == Robot.RobotStatus.Repair) {
...
//
//    if (!canRobotFire(robot, completionHandler)) return;
//
//    robot.setShots(robot.getShots() - 1); // consume shot
//
//    Position targetPosition = findHitTarget(robot);
//    Robot hitRobot = getHitRobotAt(targetPosition, robot);
//
//    if (hitRobot == null) {
...
//    hitRobot.takeHit();
//
//    Response response = createFireResponse(robot, hitRobot, targetPosition);
//    world.stateForRobot(robot, response);
//    world.stateForRobot(hitRobot, response);
//
//    completionHandler.onComplete(response);
//}
//
//private boolean canRobotFire(Robot robot, CompletionHandler handler) {
//    if (robot == null) {
//        handler.onComplete(new Response("ERROR", "Robot not found."));
//        return false;
//    }
//
//    if (robot.getShots() <= 0) {
//        handler.onComplete(new Response("ERROR", "No shots remaining."));
//        return false;
//    }
//
//    if (robot.status == Robot.RobotStatus.Reload) {
//        handler.onComplete(new Response("ERROR", robot.getName() + " is reloading and cannot fire."));
//        return false;
//    }
//
//    if (robot.status == Robot.RobotStatus.Repair) {
//        handler.onComplete(new Response("ERROR", robot.getName() + " is repairing and cannot fire."));
//        return false;
//    }
//
//    return true;
//}
//
//private Position findHitTarget(Robot robot) {
//    Position pos = robot.getPosition();
//    int range = calculateRange(robot);
//    int dx = 0, dy = 0;
//
...
//        Position target = new Position(pos.getX() + i * dx, pos.getY() + i * dy);
//        if (getHitRobotAt(target, robot) != null) {
//            return target;
//        }
//    }
//    return null;
//}
//
//private Robot getHitRobotAt(Position position, Robot shooter) {
//    for (Robot other : world.getRobots()) {
//        if (!other.getName().equals(shooter.getName()) && other.getPosition().equals(position)) {
//            return other;
//        }
//    }
//    return null;
//}
//
//private int calculateRange(Robot robot) {
//    int shots = robot.getShots();
//    return switch (robot.getMake().toLowerCase()) {
//        case "tank" -> switch (shots) {
//            case 3 -> 3; case 2 -> 4; default -> 5;
//        };
//        default -> switch (shots) {
//            case 10 -> 1; case 9 -> 2; case 8 -> 3; case 7 -> 4; case 6 -> 5;
//            case 5 -> 6; case 4 -> 7; case 3 -> 8; case 2 -> 9; default -> 10;
//        };
//    };
//}
//
//private Response createFireResponse(Robot shooter, Robot target, Position hitPosition) {
//    String message = target.isDead()
//            ? "You have hit 💥 " + target.getName() + "! It is now destroyed."
//            : "You have hit 💥 " + target.getName() + "! Remaining shield: " + target.getShields();
//
//    Response response = new Response("OK", message);
//    JSONObject data = new JSONObject();
//    data.put("message", "Hit");
//    data.put("distance", shooter.getPosition().distanceTo(hitPosition));
//    data.put("robot", target.getName());
//
//    Response hitState = new Response("OK", "I got hit");
//    world.stateForRobot(target, hitState);
//    data.put("state", hitState.object.getJSONObject("state"));
//
//    response.object.put("data", data);
//    return response;
//}
...
//}
//
//private void handleShutdown(ShutdownCommand command, String clientId, CompletionHandler completionHandler) {
//
//    String robotName = command.robot.getName();
//    if (clientRobots.containsKey(clientId)) {
//        clientRobots.get(clientId).remove(robotName);
//    }
...
//            completionHandler.onComplete(response);
//        }
//    }, world.getShieldRepairTime() * 1000L);
//}
//
